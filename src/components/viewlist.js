        ...
        <div className="h-full overflow-y-auto custom-scrollbar pr-2">
          <ul className="space-y-4">
            {filteredTodos.map((todo) => (
              <li
                key={todo.id}
                className="flex flex-col p-5 bg-[#f9fafb] rounded-xl border border-gray-100 group hover:shadow-md transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1 mr-4 min-w-0">
                    <input
                      ...
                      className="w-6 h-6 rounded-full border-2 border-gray-300 accent-[#1a6334] cursor-pointer flex-shrink-0"
                    />
                    {editingId === todo.id ? (
                      ...
                      <div className="flex flex-col min-w-0 overflow-hidden">
                        <span 
                          className={`text-xl font-medium ${
                            expandedIds.has(todo.id) ? "whitespace-normal break-words" : "truncate"
                        ...
                        {todo.text.length > 30 && (
                          <button 
                            ...
                            className="text-[#1a6334] text-sm font-bold text-left mt-1 hover:underline w-fit"
                          >
                            {expandedIds.has(todo.id) ? "Show less" : "..."}
                          </button>
                        )}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 transition-all flex-shrink-0">
                    {!editingId && (
                      <button
